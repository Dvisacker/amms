...
use crate::{
    amm::{uniswap_v2::batch_request::IGetUniswapV2PairsBatchRequest, AutomatedMarketMaker, AMM},
    errors::AMMError,
};
...
sol! {
    ...
    "src/amm/ve33/batch_request/GetVe33PoolsBatchRequestABI.json"
}
            ...
            if let Some(pool_data) = token.as_tuple() {
                ...
                if let Some(address) = pool_data[0].as_address() {
                    if !address.is_zero() {
                        ...
                        populate_v2_pool_data(ve33_pool, pool_data)?;
                        // {
                        //     tracing::trace!(?pool);
