...
impl AutomatedMarketMaker for Ve33Pool {
    ...
    fn sync_on_event_signatures(&self) -> Vec<B256> {
        vec![IVe33Pool::Sync::SIGNATURE_HASH]
    }
    ...
    fn sync_from_log(&mut self, log: Log) -> Result<(), EventLogError> {
        ...
        if event_signature == IVe33Pool::Sync::SIGNATURE_HASH {
            let sync_event = IVe33Pool::Sync::decode_log(log.as_ref(), true)?;
...
impl Ve33Pool {
        ...
        P: Provider<T, N>,
    {
        ...
        if event_signature == IVe33Factory::PairCreated::SIGNATURE_HASH {
        ...
        P: Provider<T, N>,
    {
        ...
        let v2_pair = IVe33Pool::new(self.address, provider);

        // Make a call to get the reserves
        let IVe33Pool::getReservesReturn {
            reserve0: reserve_0,
            reserve1: reserve_1,
            ..
    ...
    where
        ...
        P: Provider<T, N>,
    {
        let v2_pair = IVe33Pool::new(pair_address, provider);

        let IVe33Pool::token0Return { _0: token0 } = match v2_pair.token0().call().await {
            Ok(result) => result,
            Err(contract_error) => return Err(AMMError::ContractError(contract_error)),
        };
    ...
    where
        ...
        P: Provider<T, N>,
    {
        let v2_pair = IVe33Pool::new(pair_address, middleware);

        let IVe33Pool::token1Return { _0: token1 } = match v2_pair.token1().call().await {
            Ok(result) => result,
            Err(contract_error) => return Err(AMMError::ContractError(contract_error)),
        };
