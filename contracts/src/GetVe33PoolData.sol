...
contract GetVe33PoolData {
    ...
    constructor(address[] memory pools) {
        ...
        for (uint256 i = 0; i < pools.length; ++i) {
            ...
            try IAerodromePool(poolAddress).factory() returns (address _factory) {
            ...
            try IAerodromePool(poolAddress).getReserves() returns (
                ...
                if (_reserve0 > type(uint256).max || _reserve1 > type(uint256).max) {
                    poolData.reserve0 = 0;
                    poolData.reserve1 = 0;
                } else {
                    poolData.reserve0 = uint256(_reserve0);
                    poolData.reserve1 = uint256(_reserve1);
                }
            } catch {
                poolData.reserve0 = 0;
                poolData.reserve1 = 0;
